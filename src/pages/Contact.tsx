...
export default function Contact() {
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      // Check if the device is a mobile device using screen.width instead of window.innerWidth
      const isMobile = window.screen.width <= 768;
      setIsMobileScreen(isMobile);

      // Adjust hero height based on screen dimensions and orientation
      const isPortrait = window.screen.height > window.screen.width;
      if (isMobile) {
        setHeroHeight(isPortrait ? 'h-[45vh]' : 'h-[60vh]');
      } else {
